...
class TestDimension(unittest.TestCase):
  ...
  def test_real_transform(self):
    ...
    np.testing.assert_array_equal(dimension.inverse_transform([0.5, 1]),
                                  np.array([5.5, 10]))
...
class TestSpace(unittest.TestCase):
  ...
  def test_gowers_distance_max(self):
    distance = SPACE_1.gowers_distance([100, 100, 0, 'cat'],
                                       [-100, -100, 1, 'dog'])
  ...
  def test_gowers_difference_one_to_one(self):
    ...
    result = SPACE_1.gowers_distance(point_a, point_b)
    expected = np.array(1)
  ...
  def test_gowers_difference_one_to_many(self):
    ...
    result = SPACE_1.gowers_difference(point_a, [point_b, point_c])
  ...
  def test_numerical_columns(self):
    result = SPACE_1._numerical_columns()
    expected = [True, True, True, False]
    self.assertEqual(result, expected)

  def test_categorical_columns(self):
    result = SPACE_1._categorical_columns()
