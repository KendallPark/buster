...
class ClassificationOptimizer(optimizer.Optimizer):
  ...
  def ask(self, n_points=None, strategy="ldn"):
    ...
    neighbors = neighbors[:, self._non_cat_inds].astype(float)

    mins = neighbors.min(axis=0)
    maxes = neighbors.max(axis=0)

    new_dimensions = []
    for index, dimension in enumerate(self.space):
...
# class AdaptiveSampler(sampler.base.InitialPointGenerator):

#   def __init__(self,
#                eval_fn: Callable[[pd.DataFrame], npt.ArrayLike],
#                space: data.Space,
#                explore='lhs',
#                exploit='ldn',
#                ratio=0.5) -> None:
...
#   def generate(self, dimensions, n_samples, random_state):
#     return super().generate(dimensions, n_samples, random_state=random_state)
...
# class LargestDiverseNeighborhood(sampler.base.InitialPointGenerator):
...
#       sampler: Union[Text, sampler.base.InitialPointGenerator] = 'lhs') -> None:

#     self.data = data
#     self.labels = labels

#     if sampler == 'lhs':
#       self.sampler = sampler.lhs.Lhs()
#     else:
#       self.sampler = sampler

#   def add_samples(self, X: npt.ArrayLike, y: npt.ArrayLike):
#     pass

#   # def

#   def generate(self, dimensions, n_samples, random_state: Optional[int] = None):
