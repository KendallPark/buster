# Base code from https://github.com/wwwjk366/gower repo (MIT license) which 
...
def gower_matrix(data_x, data_y=None, weight=None, cat_features=None):  
    ...
    for col in range(num_cols):
        ...
        if np.isnan(min_val):
            min_val = 0.0
        num_max[col] = max_val
        num_ranges[col] = abs(1 - min_val / max_val) if (max_val != 0) else 0.0
...
def gower_get(xi_cat, xi_num, xj_cat, xj_num, feature_weight_cat,
              feature_weight_num, feature_weight_sum,
              ranges_of_numeric):
